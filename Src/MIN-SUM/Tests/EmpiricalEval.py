...
try:
		...
		for h in range(1, n, 20):
			for p in range(2, n, 10):
				for eps in range(1, n // p, 10):
					...
					t = time() - start
					print("   std: ", t, "s")
					# t = n^y * p^k * eps^q => log(t) = ylog(n) + klog(p) + qlog(eps)
					As.append([log(n), log(p), log(eps)])
					bs.append(log(t))
					CheckUtils.checkConsistency(OUTPUTS, n, w, T, p, eps, lambda a, b: a + b)
					
					start = time()
					os.system(SCRIPTD + " < " + INPUT + " > " + OUTPUTD)
					t = time() - start
					print("   diam:", t, "s")
					# t = n^y * min(p, h)^z * p^k * eps^q => log(t) = ylog(n) + zlog(min(p, h)) + klog(p) + qlog(eps)
					Ad.append([log(n), log(min(p, h)), log(p), log(eps)])
					bd.append(log(t))
					CheckUtils.checkConsistency(OUTPUTD, n, w, T, p, eps, lambda a, b: a + b)
	...
	print("std: ", " n^(", coeffs[0], ") p^(", coeffs[1], ") eps^(", coeffs[2], ")")
	print("diam:", " n^(", coeffd[0], ") min(p, h)^(", coeffd[1], ") p^(", coeffd[2], ") eps^(", coeffd[3], ")")
